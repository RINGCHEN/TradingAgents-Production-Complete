...
def process_news_to_jsonl(output_filename="news_articles_processed.jsonl"):
    """
    Reads raw news articles from JSONL, generates prompt-completion pairs,
    and saves them to a new JSONL file. Includes caching.
    """
    print("Starting news article processing...")
    ...
    input_dir = os.path.join(storage_base, raw_subdir, 'news_articles')
    input_jsonl_path = os.path.join(input_dir, 'news_articles.jsonl')
    ...
    positive_keywords = ["上漲", "成長", "增加", "利多", "創新高", "突破", "獲利", "看好", "樂觀", "買超", "勁揚", "飆漲", "受惠"]
    negative_keywords = ["下跌", "衰退", "減少", "利空", "破底", "跌破", "虧損", "看淡", "悲觀", "賣超", "重挫", "崩跌", "衝擊"]

    all_processed_examples = []

    try:
        print(f"Reading raw news data from {input_jsonl_path}")
        with open(input_jsonl_path, 'r', encoding='utf-8') as f_in:
            for line in f_in:
                try:
                    ...
                    # --- Create Prompt ---
                    # Limit content length for prompt to avoid excessively long inputs
                    prompt_content = content[:500] + "..." if len(content) > 500 else content
                    prompt_text = f"請根據以下新聞內容，判斷其對股市的影響是利多、利空還是中性，並簡要說明原因：\n標題：{title}\n內容：{prompt_content}"
                    ...
                    if pos_count > neg_count:
                        sentiment = "利多"
                        reason = "新聞內容中包含較多正面詞彙，暗示對股市有積極影響。"
                    elif neg_count > pos_count:
                        sentiment = "利空"
                        reason = "新聞內容中包含較多負面詞彙，暗示對股市有消極影響。"
                    else:
                        reason = "新聞內容中正面與負面詞彙數量相近，或無明顯傾向，判斷為中性影響。"
                    ...
                    print(f"Warning: Error processing news item: {e}. Skipping.")
                    continue

    except FileNotFoundError:
        print(f"Error: Input file not found at {input_jsonl_path}. Please run the news article scraper first.")
        return
    except Exception as e:
    ...
    print(f"\nSuccessfully processed {len(all_processed_examples)} news articles and saved to: {output_jsonl_path}")

if __name__ == '__main__':
    process_news_to_jsonl()
