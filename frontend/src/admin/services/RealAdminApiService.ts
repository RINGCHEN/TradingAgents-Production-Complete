...
import {
  ...
  ApiResponse as ApiClientResponse,
...
export class RealAdminApiService {
  ...
  private clearAuthData() {
    ...
    console.log('🧹 已清除所有認證信息（token、user、role）');
   ...
   * 統一處理 API 響應 - 自動處理 401 token 刷新（返回 data）
   ...
   * - ApiClient 遇到 4xx/5xx 返回 { success: false, status, error } 而不拋出異常



   * - 原代碼直接返回 response.data，未檢查 success，導致 401 時返回 undefined



   * - AdminAuthManager.handleApiError 無法被觸發
   ...
   * - 401 時自動調用 refreshAccessToken() 並重試



   * - 刷新失敗時調用 clearAuthData() 清除所有認證信息



   * - 失敗時拋出原始 ApiError（保持型態一致性）



   * - 接受合法的空響應（204 No Content 等）
   ...
   * 適用於：返回純數據的 API（如 getAnalystsInfo、getUsers 等）
   ...
   * @param retryOnAuth 是否在 401 時自動刷新（默認 true，重試時為 false 避免無限循環）



   * @returns API 響應數據（可能為 null/undefined）



   * @throws ApiError 保持原始錯誤型態，供 AdminAuthManager 處理
  ...
  private async handleApiResponseData<T>(
    ...
    // ✅ 成功：返回數據（包括合法的空響應如 204 No Content）
    ...
    if (response.success) {
      ...
      // 接受 undefined/null 作為合法數據



      return response.data !== undefined ? response.data : (null as T);
    ...
    if (response.status === 401 && retryOnAuth) {
      ...
      console.log('🔄 檢測到 401（token 過期），嘗試自動刷新...');
      ...
      if (refreshed) {
        ...
        // 重試請求（retryOnAuth = false 避免無限循環）



        return await this.handleApiResponseData(apiCall, false);
        ...
        console.log('❌ Token 刷新失敗，清除所有認證信息');
    ...
    if (response.error) {
      ...
      console.error(`❌ API 錯誤 [${response.status}]:`, response.error);



      throw response.error;  // 🚨 必須拋出，讓調用方有機會觸發 AdminAuthManager
   ...
   * 統一處理 API 響應 - 自動處理 401 token 刷新（返回完整 response）
   ...
   * 適用於：需要完整 response payload 的 API（如 deleteUser、updateUser 等）



   * 這些 API 的調用方需要檢查 success、message 等字段
   ...
   * - 刷新失敗後拋出 ApiError 而不是返回 401 response
   ...
   * @param retryOnAuth 是否在 401 時自動刷新



   * @returns 完整 API 響應（包含 success, data, message 等）



   * @throws ApiError 當 401 刷新失敗時拋出，供 AdminAuthManager 處理
  ...
  private async handleApiResponseFull<T>(
    ...
    if (response.status === 401 && retryOnAuth) {
      ...
      if (refreshed) {
        ...
        return await this.handleApiResponseFull(apiCall, false);
        ...
        // 🚨 拋出 ApiError 而不是返回 401 response
  ...
  async checkAuthStatus(): Promise<AuthStatus> {
    ...
    try {
      ...
      return await this.handleApiResponseData<AuthStatus>(



        () => this.apiClient.get<AuthStatus>('/admin/auth/verify')
      ...
      console.error('獲取系統性能總覽失敗:', error);
  ...
  async register(userData: {
    ...
    try {
      ...
      return await this.handleApiResponseData<RealUser>(



        () => this.apiClient.post<RealUser>('/register', userData)
  ...
  async getUsers(params?: {
    ...
    try {
      ...
      if (params?.search) queryParams.append('keyword', params.search); // 使用keyword取代search
      ...
      const responseData = await this.handleApiResponseData<{
      ...
      return {
        ...
        users: (responseData?.items || []).map(mapBackendUserToFrontend),
  ...
  async createUser(userData: {
    ...
    try {
      ...
      const response = await this.handleApiResponseFull(



        () => this.apiClient.post('/admin/users/', userData)
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || response.message || '用戶創建失敗';
  ...
  async updateUser(userId: string, userData: {
    ...
    try {
      ...
      const response = await this.handleApiResponseFull(
        ...
        () => this.apiClient.put(`/admin/users/${userId}`, userData)
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || response.message || '用戶更新失敗';
  ...
  async deleteUser(userId: string): Promise<{
    ...
    try {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || response.message || '用戶刪除失敗';
      ...
      return {
        ...
        message: response.message || '用戶刪除成功',
  ...
  async getUser(userId: string): Promise<User> {
    ...
    try {
      ...
      const responseData = await this.handleApiResponseData<RealUser>(
      ...
      return mapBackendUserToFrontend(responseData);
  ...
  async getUserSubscription(userId: string): Promise<RealUser['subscription']> {
    ...
    try {
      ...
      return await this.handleApiResponseData<RealUser['subscription']>(



        () => this.apiClient.get<RealUser['subscription']>(`/admin/users/${userId}/subscription`)
  ...
  async getUserLifecycle(userId: string): Promise<RealUser['lifecycle']> {
    ...
    try {
      ...
      return await this.handleApiResponseData<RealUser['lifecycle']>(



        () => this.apiClient.get<RealUser['lifecycle']>(`/users/${userId}/lifecycle`)
      ...
      console.error('獲取用戶生命周期失敗:', error);
  ...
  async getMembershipTiers(): Promise<MembershipTier[]> {
    ...
    try {
      ...
      return await this.handleApiResponseData<MembershipTier[]>(



        () => this.apiClient.get<MembershipTier[]>('/admin/users/metadata/membership-tiers')
  ...
  async getAuthProviders(): Promise<Array<{
    ...
    try {
      ...
      return await this.handleApiResponseData<Array<{
      ...
      }>>(() => this.apiClient.get('/admin/users/metadata/auth-providers'));
      ...
      return [
        ...
        { id: 'email', name: 'Email/Password', type: 'local', enabled: true }
  ...
  async getPaymentWillingness(): Promise<{
    ...
    try {
      ...
      return await this.handleApiResponseData<{
      ...
      }>(() => this.apiClient.get('/payment-willingness'));
  ...
  async getSubscriptionList(): Promise<{
    ...
    try {
      ...
      return await this.handleApiResponseData<{
        ...
        () => this.apiClient.get('/admin/subscription/list')
  ...
  async getAdminSystemStatus(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/system/status'));
    } catch (error) {
      console.error('?��?管�?系統?�?�失??', error);
  ...
  async getSystemErrorStats(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/system/error-stats'));
  ...
  async getSystemMetrics(): Promise<{
    ...
    try {
      ...
      return await this.handleApiResponseData<{
        ...
        () => this.apiClient.get('/admin/system/metrics/system')
  ...
  async getAdminAnalyticsDashboard(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/analytics/dashboard'));
   ...
   * ✅ 已使用統一 API 處理器，支持 401 自動刷新



   * ✅ 401 錯誤會重新拋出供 AdminAuthManager 處理
  ...
  async getAnalystsInfo(): Promise<{
    ...
    try {
      ...
      return await this.handleApiResponseData(
        ...
        () => this.apiClient.get('/admin/analysts/registry')
      ...
      // handleApiResponseData 拋出的是 ApiError
      ...
      if (error && typeof error === 'object' && 'status' in error) {
        ...
        if (apiError.status === 401) {
          ...
          console.error('🚨 認證失敗（401），重新拋出錯誤供 AdminAuthManager 處理');



          throw error;  // 讓 AdminApp 捕獲並觸發重定向
  ...
  async getAnalystsStatus(): Promise<{
    ...
    try {
      ...
      return await this.handleApiResponseData(
        ...
        () => this.apiClient.get('/admin/analysts/coordinator/health')
  ...
  async startAnalysisTask(analysisRequest: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/analysts/analysis', analysisRequest));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '無法啟動分析任務';
  ...
  async getAnalysisStatus(sessionId: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get(`/admin/analysts/analysis/${sessionId}`));
  ...
  async createAnalysisTask(task: {
    ...
    try {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '無法創建分析任務';
      ...
      return {
        ...
        task_id: data.execution_id || `task_${Date.now()}`,
  ...
  async getAnalysisTasks(params?: {
    ...
    try {
      ...
      console.error('獲取分析任務列表失敗:', error);
  ...
  async getAnalysisTaskDetails(taskId: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get(`/admin/analysts/analysis/${taskId}`));
      ...
      throw new Error('無法獲取分析任務詳情: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getTTSStats(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/tts/stats'));
  ...
  async getTTSQueueStatus(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/tts/queue-status'));
  ...
  async createTTSVoice(voiceData: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/tts/voices', voiceData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '創建TTS語音失敗';
  ...
  async updateTTSVoice(voiceId: string, voiceData: {
    ...
    try {
      ...
      }>(() => this.apiClient.put(`/admin/tts/voices/${voiceId}`, voiceData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '更新TTS語音失敗';
  ...
  async deleteTTSVoice(voiceId: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.delete(`/admin/tts/voices/${voiceId}`));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '刪除TTS語音失敗';
  ...
  async createArticle(articleData: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/content/articles', articleData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '創建文章失敗';
  ...
  async updateArticle(articleId: number, articleData: {
    ...
    try {
      ...
      }>(() => this.apiClient.put(`/admin/content/articles/${articleId}`, articleData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '更新文章失敗';
  ...
  async deleteArticle(articleId: number): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.delete(`/admin/content/articles/${articleId}`));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '刪除文章失敗';
  ...
  async createContentCategory(categoryData: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/content/categories', categoryData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '創建內容分類失敗';
  ...
  async createContentTag(tagData: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/content/tags', tagData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '創建內容標籤失敗';
  ...
  async getTradingStats(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/trading/stats'));
  ...
  async getTradingRiskMetrics(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/trading/risk-metrics'));
  ...
  async getDevOpsHealthOverview(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/devops/health/overview'));
  ...
  async getSubscriptionStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        total_subscriptions: toNumber(data.total_subscriptions),



        active_subscriptions: toNumber(data.active_subscriptions),



        expired_subscriptions: toNumber(data.expired_subscriptions),



        cancelled_subscriptions: toNumber(data.cancelled_subscriptions),
        ...
        monthly_revenue: toNumber(data.monthly_revenue),



        churn_rate: toNumber(data.churn_rate),



        growth_rate: toNumber(data.growth_rate),



        new_subscriptions: toNumber(



          (data as Record<string, unknown>).new_subscriptions ?? (data as Record<string, unknown>).new_users
        ...
        subscription_growth_rate: toNumber(



          (data as Record<string, unknown>).subscription_growth_rate ?? data.growth_rate
  ...
  async getActiveThreats(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/security/threats/active'));
  ...
  async getApplicationMetrics(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/system/metrics/application'));
  ...
  async getConfigItems(params?: {
    ...
    try {
      ...
      const url = queryString ? `/admin/config/items?${queryString}` : '/admin/config/items';
  ...
  async getConfigStatistics(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/config/statistics'));
  ...
  async getRevenueReport(startDate?: string, endDate?: string): Promise<{
    ...
    try {
      ...
      const url = queryString ? `/admin/subscriptions/revenue?${queryString}` : '/admin/subscriptions/revenue';
  ...
  async getFinancialDashboard(): Promise<{
    ...
    try {
      ...
      const normalizedPayment = {
        ...
        successful_payments: paymentData.successful_payments ?? 0,
        ...
        payment_success_rate: paymentData.conversion_rate ?? 0,



        average_payment_amount: paymentData.average_payment_amount ?? 0
      ...
      return {
        ...
        subscription_metrics: {
          ...
          subscription_growth_rate: subscriptionStats.subscription_growth_rate
        ...
        payment_metrics: {
          ...
          successful_payments: normalizedPayment.successful_payments,



          failed_payments: normalizedPayment.failed_payments,



          payment_success_rate: normalizedPayment.payment_success_rate,



          average_payment_amount: normalizedPayment.average_payment_amount
  ...
  async getFinancialAlerts(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/finance/alerts'));
  ...
  async getPermissionOverview(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/permissions/overview'));
  ...
  async getAllRoles(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/roles'));
  ...
  async getUserPermissions(userId: string): Promise<{
    ...
    try {
      ...
      return await this.handleApiResponseData<{
        ...
        () => this.apiClient.get(`/admin/users/${userId}/permissions`)
  ...
  async getPermissionMatrix(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/permissions/matrix'));
  ...
  async assignRoleToUser(userId: string, roleId: string, reason?: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.post(`/admin/users/${userId}/roles`, {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to assign role to user';
  ...
  async revokeRoleFromUser(userId: string, roleId: string, reason?: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.delete(`/admin/users/${userId}/roles/${roleId}`, {



        body: { reason: reason || 'Admin revocation' }
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to revoke user role';
  ...
  async getConfigOverview(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/config/overview'));
  ...
  async getAllConfigs(params?: {
    ...
    try {
      ...
      const url = queryString ? `/admin/config/settings?${queryString}` : '/admin/config/settings';
  ...
  async updateConfig(configId: string, value: string, reason?: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.put(`/admin/config/settings/${configId}`, {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '更新配置項失敗';
      ...
      throw new Error('無法更新配置項: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async createConfig(configData: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/config/settings', configData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '創建配置項失敗';
      ...
      throw new Error('無法創建配置項: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async deleteConfig(configId: string, reason?: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.delete(`/admin/config/settings/${configId}`, {



        body: { reason: reason || 'Admin deletion' }
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '刪除配置項失敗';
      ...
      throw new Error('無法刪除配置項: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async getConfigHistory(params?: {
    ...
    try {
      ...
      const url = queryString ? `/admin/config/history?${queryString}` : '/admin/config/history';
  ...
  async bulkUpdateConfigs(updates: Array<{
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/config/bulk-update', {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '批量更新配置失敗';
      ...
      throw new Error('無法批量更新配置: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async validateConfigValue(configKey: string, value: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/config/validate', {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '驗證配置值失敗';
  ...
  async exportConfigs(params?: {
    ...
    try {
      ...
      }>(() => this.apiClient.get(`/admin/config/export?${queryParams.toString()}`));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '匯出配置失敗';
      ...
      throw new Error('無法匯出配置: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getAdvancedAnalyticsDashboard(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/analytics/advanced'));
      ...
      console.error('Failed to fetch advanced analytics dashboard:', error);
  ...
  async getExperiments(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/experiments'));
  ...
  async getFeatureFlags(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/feature-flags'));
  ...
  async toggleFeatureFlag(flagKey: string, enabled: boolean, environment?: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.put(`/admin/feature-flags/${flagKey}/toggle`, {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '切換功能開關失敗';
      ...
      throw new Error('無法切換功能開關: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async emergencyDisableFeatureFlag(flagKey: string, reason: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.post(`/admin/feature-flags/${flagKey}/emergency-disable`, {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '緊急停用功能開關失敗';
      ...
      throw new Error('無法緊急停用功能開關: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async createWorkflow(workflowData: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/workflows', workflowData));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '創建工作流程失敗';
      ...
      throw new Error('無法創建工作流程: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async updateWorkflow(workflowId: string, updates: {
    ...
    try {
      ...
      const response = await this.handleApiResponseFull<{ success: boolean }>(



        () => this.apiClient.put(`/admin/workflows/${workflowId}`, updates)
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '更新工作流程失敗';
  ...
  async deleteWorkflow(workflowId: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.handleApiResponseFull<{ success: boolean }>(
        ...
        () => this.apiClient.delete(`/admin/workflows/${workflowId}`)
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '刪除工作流程失敗';
  ...
  async executeWorkflow(workflowId: string, parameters?: any): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.post(`/admin/workflows/${workflowId}/execute`, { parameters }));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '執行工作流程失敗';
      ...
      throw new Error('無法執行工作流程: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async getWorkflowExecutions(workflowId: string, params?: {
    ...
    try {
      ...
      const url = queryString
        ...
        ? `/admin/workflows/${workflowId}/executions?${queryString}`
  ...
  async cancelWorkflowExecution(executionId: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.handleApiResponseFull<{ success: boolean }>(
        ...
        () => this.apiClient.post(`/admin/workflows/executions/${executionId}/cancel`)
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '停止工作流程執行失敗';
  ...
  async getWorkflowTemplates(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/workflows/templates'));
  ...
  async createWorkflowFromTemplate(templateId: string, customizations: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/workflows/from-template', {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '從模板創建工作流程失敗';
      ...
      throw new Error('無法從模板創建工作流程: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async getWorkflowAnalytics(params?: {
    ...
    try {
      ...
      const url = queryString ? `/admin/workflows/analytics?${queryString}` : '/admin/workflows/analytics';
  ...
  async bulkWorkflowOperations(operation: 'activate' | 'deactivate' | 'delete' | 'export', workflowIds: string[]): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/workflows/bulk-operations', {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '批量工作流程操作失敗';
      ...
      throw new Error('無法執行批量工作流程操作: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async getAIOptimizationDashboard(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/ai-optimization/dashboard'));
      ...
      console.error('Failed to fetch AI optimization dashboard:', error);
  ...
  async runAIDiagnosis(components?: string[]): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/ai-optimization/diagnose', {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to run AI diagnosis';
      ...
      console.error('Failed to run AI diagnosis:', error);
      ...
      throw new Error('Unable to run AI diagnosis: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getSmartRecommendations(params?: {
    ...
    try {
      ...
      const url = queryString
        ...
        ? `/admin/ai-optimization/recommendations?${queryString}`



        : '/admin/ai-optimization/recommendations';
  ...
  async applyRecommendation(recommendationId: string, options?: {
    ...
    try {
      ...
      }>(() => this.apiClient.post(`/admin/ai-optimization/recommendations/${recommendationId}/apply`, {
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to apply recommendation';
      ...
      throw new Error('Unable to apply recommendation: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getPredictiveAnalysis(params?: {
    ...
    try {
      ...
      const url = queryString
        ...
        ? `/admin/ai-optimization/predictions?${queryString}`



        : '/admin/ai-optimization/predictions';
  ...
  async enableAutoOptimization(config: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/ai-optimization/auto-optimize', config));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to enable auto optimization';
      ...
      console.error('Failed to enable auto optimization:', error);
      ...
      throw new Error('Unable to enable auto optimization: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getOptimizationHistory(params?: {
    ...
    try {
      ...
      const url = queryString
        ...
        ? `/admin/ai-optimization/history?${queryString}`



        : '/admin/ai-optimization/history';
  ...
  async triggerManualOptimization(request: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/ai-optimization/manual-optimize', request));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to trigger manual optimization';
      ...
      console.error('Failed to trigger manual optimization:', error);
      ...
      throw new Error('Unable to trigger manual optimization: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getAIModelMetrics(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/ai-optimization/model-metrics'));
      ...
      console.error('Failed to fetch AI model metrics:', error);
      ...
      return {
        ...
        overall_ai_health: {
          ...
          recommendations_acceptance_rate: 0,
  ...
  async getAIModels(): Promise<{
    ...
    this.logApiCall('/admin/ai/models', 'GET');
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/ai/models'));
      ...
      console.error('獲取AI模型列表失敗:', error);
   ...
   * 對應API: GET /admin/ai/models/{modelId}
  ...
  async getAIModelDetails(modelId: string): Promise<{
    ...
    this.logApiCall(`/admin/ai/models/${modelId}`, 'GET');
    ...
    try {
      ...
      }>(() => this.apiClient.get(`/admin/ai/models/${modelId}`));
      ...
      console.error('獲取AI模型詳細資訊失敗:', error);
   ...
   * 對應API: GET /admin/ai/analysis/history
  ...
  async getAIAnalysisHistory(params?: {
    ...
    this.logApiCall('/admin/ai/analysis/history', 'GET');
    ...
    try {
      ...
      const url = queryString ? `/admin/ai/analysis/history?${queryString}` : '/admin/ai/analysis/history';
      ...
      console.error('獲取AI分析歷史失敗:', error);
   ...
   * 對應API: GET /admin/ai/models/{modelId}/performance
  ...
  async getAIPerformanceMetrics(modelId: string, timeRange?: '24h' | '7d' | '30d' | '90d'): Promise<{
    ...
    this.logApiCall(`/admin/ai/models/${modelId}/performance`, 'GET');
    ...
    try {
      ...
      const url = queryString
        ...
        ? `/admin/ai/models/${modelId}/performance?${queryString}`



        : `/admin/ai/models/${modelId}/performance`;
      ...
      console.error('獲取AI模型性能指標失敗:', error);
   ...
   * 對應API: GET /admin/analytics/reports
  ...
  async getAnalyticsReports(params?: {
    ...
    this.logApiCall('/admin/analytics/reports', 'GET');
    ...
    try {
      ...
      const url = queryString ? `/admin/analytics/reports?${queryString}` : '/admin/analytics/reports';
   ...
   * 對應API: GET /admin/analytics/insights
  ...
  async getAnalyticsInsights(params?: {
    ...
    this.logApiCall('/admin/analytics/insights', 'GET');
    ...
    try {
      ...
      const url = queryString ? `/admin/analytics/insights?${queryString}` : '/admin/analytics/insights';
   ...
   * 對應API: POST /admin/ai/models/train
  ...
  async trainAIModel(trainingConfig: {
    ...
    this.logApiCall('/admin/ai/models/train', 'POST');
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/ai/models/train', trainingConfig));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '啟動AI模型訓練失敗';
      ...
      console.error('啟動AI模型訓練失敗:', error);
      ...
      throw new Error('無法啟動AI模型訓練: ' + (error instanceof Error ? error.message : '未知錯誤'));
   ...
   * 對應API: PUT /admin/ai/models/{modelId}/config
  ...
  async updateAIModelConfig(modelId: string, config: {
    ...
    this.logApiCall(`/admin/ai/models/${modelId}/config`, 'PUT');
    ...
    try {
      ...
      }>(() => this.apiClient.put(`/admin/ai/models/${modelId}/config`, config));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '更新AI模型配置失敗';
      ...
      console.error('更新AI模型配置失敗:', error);
      ...
      throw new Error('無法更新AI模型配置: ' + (error instanceof Error ? error.message : '未知錯誤'));
   ...
   * 對應API: POST /admin/ai/models/{modelId}/deploy
  ...
  async deployAIModel(modelId: string, deploymentConfig: {
    ...
    this.logApiCall(`/admin/ai/models/${modelId}/deploy`, 'POST');
    ...
    try {
      ...
      }>(() => this.apiClient.post(`/admin/ai/models/${modelId}/deploy`, deploymentConfig));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '部署AI模型失敗';
      ...
      throw new Error('無法部署AI模型: ' + (error instanceof Error ? error.message : '未知錯誤'));
   ...
   * 對應API: POST /admin/analytics/reports
  ...
  async generateAnalyticsReport(reportConfig: {
    ...
    report_type: 'user_behavior' | 'financial' | 'ai_performance' | 'system_health';
    ...
    this.logApiCall('/admin/analytics/reports', 'POST');
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/analytics/reports', reportConfig));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || '生成分析報告失敗';
      ...
      throw new Error('無法生成分析報告: ' + (error instanceof Error ? error.message : '未知錯誤'));
  ...
  async getPerformanceOverview(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/performance/overview'));
  ...
  async getRealtimePerformance(params?: {
    ...
    try {
      ...
      const url = queryString ? `/admin/performance/realtime?${queryString}` : '/admin/performance/realtime';
      ...
      console.error('獲取即時性能指標失敗:', error);
  ...
  async runLoadTest(config: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/performance/load-test', config));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to run performance load test';
      ...
      console.error('Failed to run performance load test:', error);
      ...
      throw new Error('Unable to start performance load test: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getLoadTestResults(testId: string): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get(`/admin/performance/load-test/${testId}/results`));
  ...
  async configureAutoScaling(config: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/performance/auto-scaling/configure', config));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to configure auto scaling';
      ...
      throw new Error('Unable to configure auto scaling: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getAutoScalingHistory(params?: {
    ...
    try {
      ...
      const url = queryString ? `/admin/performance/auto-scaling/history?${queryString}` : '/admin/performance/auto-scaling/history';
      ...
      console.error('Failed to fetch auto scaling history:', error);
  ...
  async runOptimizationAnalysis(scope?: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/performance/optimization-analysis', scope || {}));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to run optimization analysis';
      ...
      throw new Error('Unable to run optimization analysis: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getCacheStatistics(cache_type?: 'redis' | 'memcached' | 'application' | 'all'): Promise<{
    ...
    try {
      ...
      const url = queryString ? `/admin/performance/cache/statistics?${queryString}` : '/admin/performance/cache/statistics';
  ...
  async optimizeCacheConfiguration(optimization: {
    ...
    try {
      ...
      }>(() => this.apiClient.post('/admin/performance/cache/optimize', optimization));
      ...
      if (!response.success) {
        ...
        const errorMessage = response.error?.message || 'Failed to optimize cache configuration';
      ...
      throw new Error('Unable to optimize cache configuration: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  async getDatabasePerformanceAnalysis(): Promise<{
    ...
    try {
      ...
      }>(() => this.apiClient.get('/admin/performance/database/analysis'));
      ...
      console.error('Failed to fetch database performance analysis:', error);
